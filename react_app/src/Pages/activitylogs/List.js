...
function Activity({ activityData, totalPages, currentPage,handleNextPage,pageNumbers }) {
 return (
    <div className="main">
            <div className="main-content">
                    <div className=" bold ft-23 mb-3">Activity Log</div>
                  <div className="table-responsive">
                        <table className="table table-bordered table-hover">
                              ...
                              <tbody>
                                ...
                                <tr key={item.id}>
                                    <td>{['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][new Date(item.timestamp).getMonth()]} {new Date(item.timestamp).getDate()}, {new Date(item.timestamp).getFullYear()} {new Date(item.timestamp).toLocaleTimeString()}</td>
                                    <td>{item.added_by}</td>
                                    <td>{item.username}</td>
